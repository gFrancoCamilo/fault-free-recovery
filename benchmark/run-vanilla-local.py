        ...
        with open("fabfile.py", "w") as file:
            for line in lines:
                if '"rate":' in line:
                    ...
                    file.write(f'        "duration": 60,\n')
                else:
                    file.write(line)

        # Run the benchmark for the specified number of runs
        for run in range(1, num_runs + 1):
            output_file = f"results/bench-0-{num_nodes}-{rate}-512.txt"
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python run-vanilla-local.py <smallest_rate> <highest_rate> <num_nodes> <num_runs>")
        sys.exit(1)
